const { Client, GatewayIntentBits, PermissionsBitField, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, MessageAttachment } = require('discord.js'); const QRCode = require('qrcode');

const express = require("express"); const app = express(); app.get("/", (req, res) => res.send("PLAY BOY E-SPORTS ONLINE")); app.listen(process.env.PORT || 3000);

const client = new Client({ intents: [ GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages, GatewayIntentBits.MessageContent, GatewayIntentBits.GuildMembers ] });

const TOKEN = process.env.TOKEN;

// ===================== // FILAS, PREÇOS E ESTADOS // ===================== let filas = { "x1-mobile": [], "x2-mobile": [], "x3-mobile": [], "x4-mobile": [], "x1-emulador": [], "x2-emulador": [], "x3-emulador": [], "x4-emulador": [] };

let precos = [0.30,0.50,0.70,1,2,3,5,10,20,30,50,70,100]; let canaisPrivados = {}; let painelMsg = {}; let senhas = {}; let pixInfo = {}; const cargosRestritos = ["DONO","ADMIN GERAL","GERENTE","SUPORTE","STAFF"];

client.once("ready", () => console.log("👑 PLAY BOY E-SPORTS ONLINE"));

function delay(ms){ return new Promise(resolve=>setTimeout(resolve, ms)); }

// ===================== // COMANDO !setup // ===================== client.on("messageCreate", async message => { if(!message.guild) return; if(message.content === "!setup"){ if(!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) return message.reply("❌ Você precisa ser administrador.");
...
for(const [catName, modos] of Object.entries(categorias)){
  ...
  for(let modo of modos){
    ...
    // ======= BOTÕES DE PREÇO + AÇÃO (separados) =======
    for(let valor of precos){
      ...
      let painel = await canal.send({content:`👑 FILA ${modo.toUpperCase()} - R$${valor}

Escolha sua ação:, components:[row]}); painelMsg[${modo}_${valor}`] = painel.id; await delay(200); } } }
...
// ================= RESET ================= if(message.content === "!reset"){ if(!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) return message.reply("❌ Apenas ADM pode resetar."); message.reply("🗑️ Excluindo todos os canais..."); message.guild.channels.cache.forEach(c => {if(c.deletable) c.delete().catch(()=>{});}); }

// ================= COMANDO CADASTRO PIX ================= if(message.content.startsWith("!pix")){ const memberRoles = message.member.roles.cache.map(r=>r.name.toUpperCase()); if(!memberRoles.some(r=>cargosRestritos.includes(r))) return message.reply("❌ Apenas ADM pode cadastrar Pix."); const chave = message.content.split(" ")[1]; if(!chave) return message.reply("❌ Digite a chave do Pix após o comando !pix <chave>."); pixInfo[message.guild.id] = chave;
...
// ================= INTERAÇÃO DE BOTÕES ================= client.on("interactionCreate", async interaction => { if(!interaction.isButton()) return; const userId = interaction.user.id;

// ================= BOTÕES DE PREÇO + AÇÃO ================= if(interaction.customId.includes("preco")){ const [modo,, valorStr, acao] = interaction.customId.split("_"); const valor = parseFloat(valorStr); filas[modo] = filas[modo] || [];
...
// ================= FECHAR CANAL ================= if(interaction.customId === "fechar-canal"){ const memberRoles = interaction.member.roles.cache.map(r=>r.name.toUpperCase()); if(!memberRoles.some(r=>cargosRestritos.includes(r))) return interaction.reply({content:"❌ Apenas ADM pode fechar.", ephemeral:true}); await interaction.channel.delete().catch(()=>{}); }

// ================= ACEITAR APOSTA ================= if(interaction.customId === "aceitar-aposta"){ const admRoles = cargosRestritos; const adms = interaction.guild.members.cache.filter(m=>m.roles.cache.some(r=>admRoles.includes(r.name.toUpperCase()))); const canalPriv = interaction.channel;
...
// ================= ABRIR TICKET ================= if(interaction.customId === "abrir-ticket"){ const ticketCat = interaction.guild.channels.cache.find(c=>c.name==="🎫 SUPORTE" && c.type===4); const ticketChannel = await interaction.guild.channels.create({ name:🎫-ticket-${interaction.user.username}, type:0, parent:ticketCat.id, permissionOverwrites:[ {id:interaction.guild.id, deny:[PermissionsBitField.Flags.ViewChannel]}, {id:interaction.user.id, allow:[PermissionsBitField.Flags.ViewChannel]}, {id:interaction.guild.roles.cache.find(r=>r.name==="SUPORTE")?.id, allow:[PermissionsBitField.Flags.ViewChannel]} ] }); ticketChannel.send(Olá <@${interaction.user.id}>, aguarde que a equipe de suporte atenderá você.); return interaction.reply({content:✅ Ticket criado: <#${ticketChannel.id}>, ephemeral:true}); } });

client.login(TOKEN);

// ===================== LOG DE ERROS ===================== process.on('unhandledRejection', err => console.log('Erro não tratado:', err)); process.on('uncaughtException', err => console.log('Exceção:', err));
